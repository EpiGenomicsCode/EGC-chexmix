...
import org.egc.core.deepseq.utils.simulation.CountDataSimulator;
import org.egc.core.deepseq.utils.simulation.CountDataSimulator.SimCounts;
 ...
 * @author Naomi Yamada
 *
 * Extended from org.egc.core.deepseq.utils.simulation.ChIPReadSimulator
 */
	...
	public static void main(String[] args) {
		...
		if(args.length==0 || ap.hasKey("h") || !ap.hasKey("emp")){
			...
			if (ap.hasKey("points")){
				...
				numdata = Integer.valueOf(ap.getKeyValue("numdata"));
				cdsim.setDataPoints(numdata);
			}if(ap.hasKey("c")){
				c = Integer.valueOf(ap.getKeyValue("c"));
				if(c!=1 && c!=2){
				...
				r = Integer.valueOf(ap.getKeyValue("r"));
				cdsim.setReplicates(r);
			}if(ap.hasKey("a")){
				a = Double.valueOf(ap.getKeyValue("a"));
				cdsim.setAlpha(a);
			}if(ap.hasKey("frags")){
				frags = Double.valueOf(ap.getKeyValue("frags"));
				cdsim.setReadsA(frags);
				cdsim.setReadsB(frags);
			}if(ap.hasKey("reads")){
				reads = Double.valueOf(ap.getKeyValue("reads"));
			}if(ap.hasKey("rlen")){
				rlen = Integer.valueOf(ap.getKeyValue("rlen"));
			}if(ap.hasKey("up")){
				up = Double.valueOf(ap.getKeyValue("up"));
				cdsim.setUpRegFrac(up);
			}if(ap.hasKey("down")){
				down = Double.valueOf(ap.getKeyValue("down"));
				cdsim.setDownRegFrac(down);
			}if(ap.hasKey("diff")){
				diff = Double.valueOf(ap.getKeyValue("diff"));
				cdsim.setDiffExpLevel(diff);
			}if(ap.hasKey("jointrate")){
				jointRate = Double.valueOf(ap.getKeyValue("jointrate"));
			}if(ap.hasKey("jointspacing")){
				jointSpacing = Integer.valueOf(ap.getKeyValue("jointspacing"));
			}if(ap.hasKey("out")){
				...
				overlapEvent = Integer.valueOf(ap.getKeyValue("overlap"));
			}if(ap.hasKey("eventspacing")){
				eventSpacing = Integer.valueOf(ap.getKeyValue("eventspacing"));
			}
