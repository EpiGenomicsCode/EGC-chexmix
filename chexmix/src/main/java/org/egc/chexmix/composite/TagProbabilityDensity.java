package org.egc.chexmix.composite;
...
public class TagProbabilityDensity {
	...
	public TagProbabilityDensity(File distFile){
		...
		try {
	        ...
	        while ((line = reader.readLine()) != null) {
	            ...
	            if(words.length>=2){
		              Integer dist = Integer.valueOf(words[0]);
	...
	public static TagProbabilityDensity load(List<String> lines){
		...
		if(lines.size()!=3){
			...
			r = Integer.valueOf(bits[3]);
			w = Integer.valueOf(bits[4]);
			tpd = new TagProbabilityDensity(l, r);
			tpd.setIndex(index);
			if(bits2[0].equals("GaussianW") && bits3[0].equals("GaussianC")){
				Double gow = Double.valueOf(bits2[1]);
				Double gsw = Double.valueOf(bits2[2]);
				Double goc = Double.valueOf(bits3[1]);
				Double gsc = Double.valueOf(bits3[2]);
				...
				for(int i=0; i<w; i++){
					watson[i] = Double.valueOf(bits2[i+1]);
					crick[i] = Double.valueOf(bits3[i+1]);
				}
				try {
